<script lang="ts">
    ...
    import { Button, Icon, ProgressCircular, Slider } from "svelte-materialify";
    ...
    function playOrStop(source: PCMSource) {
        if (startTime === 0) {
            ...
            const buf = source.ctx.createBuffer(1, source.source.length, source.rate)
...
{:then source}
    <div class="audio-div d-flex ma-2 mb-0 align-center align-content-center">
        <Button icon on:click={() => playOrStop(source)}>
            <Icon path={startTime === 0 ? mdiPlay : mdiStop} />
        </Button>
        <Slider thumb={startTime !== 0} style="margin:8px 18px 0 0;" bind:value={playbackPosition} max={source.length} />
        <span class="mr-2">
            {timeToString(Math.round(playbackPosition))}/{timeToString(Math.round(source.length))}
        </span>
    </div>
{:catch}
